...
class OmniAuth::Strategies::OpenIDConnect
  ...
  def callback_phase
    ...
    if error
      raise CallbackError.new(request.params['error'], request.params['error_descripion'] || request.params['error_reason'], request.params['error_uri'])
  ...
  def issuer
    resource = "#{client_options.scheme}://#{client_options.host}" + (client_options.port ? ":#{client_options.port}" : '')
